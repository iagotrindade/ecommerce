                ...
                <tr id = "user-item">
                    <th class="users-table-body-info" id="users-image-th" scope="row">
                        <a href="" class="user-image">
                            <img src="{{url("storage/{$searchUser->image}")}}" alt="Avatar">
                        </a>
                    </th>

                    <td class="table-body-info" id="user-name">{{$searchUser->name}}</td>

                    <td class="table-body-info">{{$searchUser['username']}}</td>

                    <td class="table-body-info" id="user-permission">{{$searchUser->permissionName}}</td>

                    <td class="table-body-info">
                        <p class="@if (($searchUser->status === "Ativado"))green-user-status @else red-user-status @endif default-flex" id="user-status">{{$searchUser->status}}</p>
                    </td>


                    @if($permissionsController::hasPermission('edit_user', $authUser['permissionSlugs']))
                        <td class="table-body-info" id="edit-user-button" wire:click="openEditModal({{$searchUser['id']}})" data-user="{{$userEdited}}">
                            <p>
                                <img src="{{url('assets/images/panel-icons/edit_user.png')}}" alt="Editar Usuário">
                            </p>
                        </td>
                    @elseif ($permissionsController::hasPermission('edit_my_user', $authUser['permissionSlugs']))
                        @if ($searchUser->id == $authUser['id'])
                            <td class="table-body-info" id="edit-user-button" wire:click="openEditModal({{$searchUser->id}})" data-user="{{$userEdited}}">
                                <p>
                                    <img src="{{url('assets/images/panel-icons/edit_user.png')}}" alt="Editar Usuário">
                                </p>
            ...
            <div class="edit-user-modal-area" style="display: {{$modalSatus}}">
                <form method="POST" action="{{route('users.edit')}}" enctype="multipart/form-data">
                    ...
                    <div class="default-flex-between">
                        ...
                        <div class="default-flex-between">
                            <x-buttons.default_secondary_button id="close-edit-user-modal-button" text="Cancelar" onclick="closeModal()"></x-buttons.default_secondary_button>
                            <x-buttons.default_primary_button tag="button" type="submit" text="Salvar Alterações" wire:click="sendForm"></x-buttons.default_primary_button>
                        </div>
                    </div>
                </form>
            </div>
